'use client';

import { useState } from 'react';
import { X, LogIn, ShieldCheck, Loader2 } from 'lucide-react';
import { signInWithGoogle } from '../lib/supabaseClient';

export default function LoginModal({ isOpen, onClose, onLoginSuccess }) {
  const [isLoggingIn, setIsLoggingIn] = useState(false);
  const [error, setError] = useState('');

  if (!isOpen) return null;

  const handleGoogleLogin = async () => {
    try {
      setIsLoggingIn(true);
      setError('');
      const user = await signInWithGoogle();
      if (user && onLoginSuccess) {
        onLoginSuccess(user);
      }
      onClose();
    } catch (err) {
      console.error('Google login error:', err);
      setError(err.message || 'Gagal login menggunakan Google.');
    } finally {
      setIsLoggingIn(false);
    }
  };

  return (
    <div className="modal-backdrop">
      <div className="modal-content login-modal">
        <div className="modal-header">
          <h3 className="modal-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <LogIn size={20} style={{ color: 'var(--primary)' }} />
            Masuk Akun LogBook
          </h3>
          <button onClick={onClose} className="btn-icon" aria-label="Tutup">
            <X size={20} />
          </button>
        </div>

        <div className="modal-body text-center py-6">
          <div className="login-icon-badge">
            <ShieldCheck size={36} style={{ color: 'var(--primary)' }} />
          </div>

          <h4 style={{ fontSize: '1.125rem', fontWeight: 700, marginTop: '0.75rem', color: '#0f172a' }}>
            Selamat Datang di LogBook Magang
          </h4>
          
          <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: '0.375rem', marginBottom: '1.5rem' }}>
            Masuk menggunakan akun Google Anda untuk menyimpan data logbook harian secara otomatis di cloud & sync ke HP Anda.
          </p>

          {error && <div className="alert-error" style={{ textAlign: 'left' }}>{error}</div>}

          <button
            onClick={handleGoogleLogin}
            disabled={isLoggingIn}
            className="btn btn-google-auth"
          >
            {isLoggingIn ? (
              <>
                <Loader2 size={18} className="animate-spin" /> Menghubungkan Google...
              </>
            ) : (
              <>
                <svg width="20" height="20" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"/>
                </svg>
                Masuk dengan Google
              </>
            )}
          </button>
        </div>

        <div className="modal-footer" style={{ justifyContent: 'center' }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            Data tersimpan aman & terlindungi oleh Supabase Auth
          </span>
        </div>
      </div>
    </div>
  );
}
